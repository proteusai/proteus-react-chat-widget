...
const messagesReducer = {
  [ADD_NEW_USER_MESSAGE]: (state: MessagesState, { message, showClientAvatar, id }) =>
    ({ ...state, messages: [...state.messages, createNewMessage(message, MESSAGE_SENDER.CLIENT, id)]}),

  [ADD_NEW_RESPONSE_MESSAGE]: (state: MessagesState, { message, id }) =>
    ({ ...state, messages: [...state.messages, createNewMessage(message, MESSAGE_SENDER.RESPONSE, id)], badgeCount: state.badgeCount + 1 }),
